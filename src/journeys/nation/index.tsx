...
import { FluencyScreen } from './screens/Fluency';
import type { StepResult } from '../../types';

export const NationJourney: React.FC = () => {
  ...
  if (showComplete) {
    ...
    const mastery = totalSteps > 0 && correctSteps / totalSteps >= 0.75;
    const fluencyResult = stepResults.find((r) => r.stepId === 'fluency');
    const passedFluency = fluencyResult?.correct && !fluencyResult?.skipped;
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold mb-2">
              {mastery && passedFluency ? '🎉' : '👍'} Journey Complete!
            </h1>
            <p className="text-xl text-gray-700">
              You've finished Nation's Four Strands journey for "{word.english}"
            </p>
          </div>
  ...
  const stepComponents = [
    <InputScreen key="input" word={word} onComplete={(result) => handleStepComplete(result)} />,
    <LanguageScreen key="language" word={word} onComplete={(result) => handleStepComplete(result)} />,
    <OutputScreen key="output" word={word} onComplete={(result) => handleStepComplete(result)} />,
    <FluencyScreen key="fluency" word={word} onComplete={(result) => handleStepComplete(result)} />,
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-yellow-50 to-white py-8">
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Nation's Four Strands Journey</h1>
